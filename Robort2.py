...
def cal_no_con_pairs(Question, pair_1, pair_2, prompt=None):
    if prompt != None:
        ...
        Prompt = f"下边的这两个句子是一个调查问卷的回答，请问在多大程度上他们反映出来的金融素养是一致的，请从1-5打分，1分表示很不一致，5分表示非常一致。问题是{Question},回答对分别是{pair_1}和{pair_2}。回答的格式应该是(原因,'分数')，其中原因是你为什么会做出这样的判断，分数应该是用整数表示的最终的结果。"
...
def Cal_sim(Input_pair, Question, Cal_mode_Singel=True, CoT=None, Predefined_Task_Information_Setting=None, StageI=True, domen=None):
    ...
    if Cal_mode_Singel:
        ...
        #     score.append(cal_con_pairs(Question, item1, item2))
    return score
